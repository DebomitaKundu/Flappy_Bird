...
#Global variablesfor the game which we use throughtout the game
FPS = 32        #fps i.e Frame Per Second =32 that means 32 images will blit(show) on the screen in just 1sec
SCREENWIDTH = 289   #breadth of game window
SCREENHEIGHT = 511  #length of game window
SCREEN = pygame.display.set_mode((SCREENWIDTH, SCREENHEIGHT))   #setmode initialise a window or screen for display
GROUNDY = SCREENHEIGHT*0.8  #groundy=base image
GAME_SPRITES = {}   #game_sprites dictionary created
GAME_SOUNDS = {}    #game_sounds dictionary created
PLAYER = 'resources\SPRITES\\bird.png'      #path of bird
...
def welcomeScreen():
    ...
    # Drawing  transperant Rectangle for playbutton as the icon is not ant bttn it is within the img
    playbutton = pygame.Rect(108,222,68,65) #rect() takes 4 arguments x,y,lenght,breadh

    while True:
        for event in pygame.event.get():
             #If user clicks on Cross or presses the Escape button Close the Game with the help of pygame.quit()
                #sys.exit()
             
            if event.type== QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()
             #if the userpresses space or up key start the game for them
            ...
            if playbutton.collidepoint(pygame.mouse.get_pos()): #checking if mouse is collided with the play button
                ...
                FPSCLOCK.tick(FPS)  #controls our FPS
...
def mainGame():
    ...
    pipeVelX = -4   #pipevelocity -4 as we want our pipe from right to left not the reverse
    playerVelY = -9
    playerMaxVelY = 10  
    playerMinVelY = -8
    playerAccY = 1
    ...
    while True:
        ...
        SCREEN.blit(GAME_SPRITES['base'], (basex, GROUNDY))
        SCREEN.blit(GAME_SPRITES['player'], (playerx, playery))
        myDigits = [int(x) for x in list(str(score))]   #stores list of our scores
        width = 0
...
def getRandomPipe():    
    ...
    pipe = [ 
        {'x': pipeX, 'y': -y1}, #Upper Pipes value -ve as above the screen
...
if __name__ == "__main__":
    ...
    GAME_SPRITES['message'] = pygame.image.load('resources\SPRITES\messagenew.png').convert_alpha()
    ...
    while True:
        ...
        mainGame() #This is our main game func